...
n_raw = df[df["Amplifier_Board"].astype(str) == "NBrn"].shape[0]
n_filt = temp_df[temp_df["Amplifier_Board"].astype(str) == "NBrn"].shape[0]

st.sidebar.write("DEBUG NBrn rows in raw df:", n_raw)
st.sidebar.write("DEBUG NBrn rows after filters:", n_filt)

st.write("DEBUG: raw NBrn rows")
st.dataframe(df[df["Amplifier_Board"].astype(str) == "NBrn"].head(20))

st.write("DEBUG: filtered NBrn rows")
st.dataframe(temp_df[temp_df["Amplifier_Board"].astype(str) == "NBrn"].head(20))
