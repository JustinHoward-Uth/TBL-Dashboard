...
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
            try:
                ...
                report_html = generate_report_html(filter_summary, stats_df, img_b64, "Impedance Box + Scatter")
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
...
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
            try:
                ...
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                stats_df = make_stats_table(imp_long, "Imp_Ohm", group_col)
                report_html = generate_report_html(filter_summary, stats_df, img_b64, "KDE Violin")
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
...
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
            try:
                img_bytes = fig.to_image(format="png", width=900, height=500)
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                report_html = generate_report_html(filter_summary, pd.DataFrame(), img_b64, "Channel Violin Subplots")
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
...
Amplifier Boards: {selected_boards}"""
            try:
                img_bytes = fig.to_image(format="png", width=900, height=500)
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                report_html = generate_report_html(filter_summary, low_table, img_b64, "Devices with Any Channel < 5kΩ")
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
...
Rows used: {len(temp_df)}"""
    try:
        # Build a simple text-based stats report (no chart for stats section)
        stats_html = f"""
        <html><head><style>
          body {{ font-family: Arial, sans-serif; padding: 40px; }}
          h1 {{ color: #2c3e50; }}
