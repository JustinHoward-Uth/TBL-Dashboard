...
METADATA_PATH = "metadata_descriptions.csv"
...
@st.cache_data
def load_metadata_descriptions(path=METADATA_PATH):
    try:
        ...
        meta["description"] = meta["description"].astype(str).str.strip()
        ajp_desc = (
            meta[meta["Category"] == "AJP_Pattern"]
            .drop_duplicates(subset=["code"])
            .set_index("code")["description"]
            .to_dict()
        )
        wafer_desc = (
            meta[meta["Category"] == "Wafer_Type"]
            .drop_duplicates(subset=["code"])
            .set_index("code")["description"]
            .to_dict()
        )
        return ajp_desc, wafer_desc
    except Exception:
        return {}, {}


ajp_descriptions, wafer_descriptions = load_metadata_descriptions()
...
def load_data(path):
    ...
    return df, imp_cols, rms_cols


@st.cache_data
def load_metadata_descriptions(path):
    try:
        meta = pd.read_csv(path, encoding="utf-8-sig")
        meta.columns = meta.columns.str.strip()

        required = {"Category", "code", "description"}
        ...
        ref_df = (
            meta.loc[
                meta["Category"].isin(["Wafer_Type", "AJP_Pattern"]),
                ["Category", "code", "description"],
            ]
            .drop_duplicates()
            .rename(columns={"code": "Code", "description": "Description"})
            .sort_values(["Category", "Code"])
            .reset_index(drop=True)
        )

        return ajp_desc, wafer_desc, ref_df

    except Exception:
        empty_ref = pd.DataFrame(columns=["Category", "Code", "Description"])
        return {}, {}, empty_ref


def load_data_from_dataframe(df_in):
    df = df_in.copy()
    if "Amplifier_board" in df.columns and "Amplifier_Board" not in df.columns:
        df = df.rename(columns={"Amplifier_board": "Amplifier_Board"})

    # All the same preprocessing as in load_data, but WITHOUT the pd.read_csv part
    # You can literally copy the body of load_data starting after `df = pd.read_csv(path)`
    # and before `return df, imp_cols`:
...
def plot_kde_violin_like(imp_long, group_col, title):
    ...
    }  # this creates a mapping of each group to a numeric position on the x-axis, which is used for plotting the KDE-based violins at the correct locations. The positions are assigned based on the sorted order of the unique groups in the data. it also
...
def plot_car_nocar_dumbbell(temp_df, group_col, title):
    cols_needed = [group_col, "CAR_uV_numeric", "NoCAR_uV_numeric"]
    for extra in ["Amplifier_Board", "Device_ID"]:
        if extra in temp_df.columns:
            cols_needed.append(extra)
    plot_df = temp_df[[c for c in cols_needed if c in temp_df.columns]].copy()
    plot_df = plot_df.dropna(subset=["CAR_uV_numeric", "NoCAR_uV_numeric"], how="all")
    plot_df = plot_df[
        plot_df[group_col].notna() & (plot_df[group_col].astype(str) != "Unknown")
    ]

    valid_groups = sorted(plot_df[group_col].astype(str).unique().tolist())
    assigned_colors = {}
    positions = {g: i for i, g in enumerate(valid_groups)}
    rng = np.random.default_rng(42)
    fig = go.Figure()
    ...
    paired = plot_df.dropna(subset=["CAR_uV_numeric", "NoCAR_uV_numeric"])
    for group in valid_groups:
        gdf = paired[paired[group_col].astype(str) == group].copy()
        if gdf.empty:
            continue
        base_x = positions[group]
        jitter = rng.uniform(-0.25, 0.25, len(gdf))
        gdf["_xj"] = base_x + jitter
        color = (
            resolve_wafer_color(group, assigned_colors)
            if group_col == "Wafer_Type"
        ...
        for _, row in gdf.iterrows():
            xs += [row["_xj"] - 0.06, row["_xj"] + 0.06, None]
            ys += [row["CAR_uV_numeric"], row["NoCAR_uV_numeric"], None]
        fig.add_trace(
            go.Scatter(
                x=xs,
                y=ys,
    ...
    for group in valid_groups:
        gdf = (
            plot_df[plot_df[group_col].astype(str) == group]
            .dropna(subset=["CAR_uV_numeric"])
            .copy()
        )
        if gdf.empty:
            continue
        base_x = positions[group]
        jitter_vals = rng.uniform(-0.25, 0.25, len(gdf))
        color = (
            resolve_wafer_color(group, assigned_colors)
            if group_col == "Wafer_Type"
            else None
        ...
        hover_fields = (
            np.stack([gdf["Device_ID"].fillna("NA")], axis=-1)
            if "Device_ID" in gdf.columns
            else None
        )
        fig.add_trace(
            go.Scatter(
                x=base_x + jitter_vals - 0.06,
                y=gdf["CAR_uV_numeric"],
                mode="markers",
                name=f"{group} CAR",
                marker=dict(
                    symbol="circle",
                ...
                hovertemplate=(
                    f"<b>{group}</b> — CAR<br>Value: %{{y:.3f}} µV<br>"
                    + (
    ...
    # NoCAR markers (diamonds)
    for group in valid_groups:
        gdf = (
            plot_df[plot_df[group_col].astype(str) == group]
            .dropna(subset=["NoCAR_uV_numeric"])
            .copy()
        )
        if gdf.empty:
            continue
        ...
        fig.add_trace(
            go.Scatter(
                ...
                customdata=hover_fields,
                hovertemplate=(
                    f"<b>{group}</b> — NoCAR<br>Value: %{{y:.3f}} µV<br>"
                    + (
                        "Device: %{customdata[0]}<br>"
                        if "Device_ID" in gdf.columns
                        else ""
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                ...
                xref="paper",
                yref="paper",
                text="● = CAR  |  ◆ = NoCAR  |  lines connect same device",
                showarrow=False,
...
def build_rms_long(temp_df, rms_cols):
    id_vars = [
        c
        for c in [
            "Wafer_Type",
    ...
    return pd.melt(
        ...
        var_name="Channel",
        value_name="RMS_uV",
    ).dropna(subset=["RMS_uV"])


def plot_channel_violin_subplots_rms(
    temp_df, rms_cols, group_col, rms_min, rms_max, title
):
    id_vars = [c for c in [group_col, "Amplifier_Board"] if c in temp_df.columns]
    rms_long = pd.melt(
        temp_df,
        ...
        value_vars=rms_cols,
        var_name="Channel",
        value_name="RMS_Value_uV",
    ).dropna(subset=["RMS_Value_uV"])
    rms_long = rms_long[
        (rms_long["RMS_Value_uV"] >= rms_min) & (rms_long["RMS_Value_uV"] <= rms_max)
    ]
    groups = [
        g
        for g in sorted(rms_long[group_col].dropna().unique().tolist())
        if str(g) != "Unknown"
    ]
    if not groups:
        return go.Figure()
    ...
    for i, group in enumerate(groups):
        gdf = rms_long[rms_long[group_col] == group]
        row, col = i // 2 + 1, i % 2 + 1
        fig.add_trace(
            go.Violin(
                x=gdf["Channel"],
    ...
    for r in range(1, math.ceil(len(groups) / 2) + 1):
        for c in range(1, 3):
            fig.update_yaxes(title_text="RMS (µV)", row=r, col=c)
            fig.update_xaxes(title_text="Channel", row=r, col=c)
    fig.update_layout(height=350 * math.ceil(len(groups) / 2), title=title)
    return fig
...
def generate_report_html(filter_summary, stats_df, fig_b64, plot_label):
    table_html = stats_df.to_html(index=False, border=1)
    html = f"""
    <html><head><style>
      ...
      pre {{ background: #f9f9f9; padding: 12px; border-radius: 4px; }}
    </style></head><body>
    <h1>TBL Device Analysis Report</h1>
    <p><b>Generated:</b> {pd.Timestamp.now().strftime("%Y-%m-%d %H:%M")}</p>
    <h2>Filter Settings</h2>
    <pre>{filter_summary}</pre>
    <h2>{plot_label}</h2>
    <img src="data:image/png;base64,{fig_b64}" width="100%"/>
...
ajp_descriptions, wafer_descriptions, metadata_reference_df = (
    load_metadata_descriptions(METADATA_PATH)
)

st.sidebar.markdown("### Data Source")

uploaded_file = st.sidebar.file_uploader(
    "Upload device summary CSV",
    type=["csv"],
    help="Upload a CSV with the required device summary columns.",
)
...
if "Amplifier_board" in raw_df.columns and "Amplifier_Board" not in raw_df.columns:
    raw_df = raw_df.rename(columns={"Amplifier_board": "Amplifier_Board"})
...
df, imp_cols, rms_cols = load_data_from_dataframe(raw_df)
...
with st.expander("AJP Pattern and Wafer Reference", expanded=False):
    if not metadata_reference_df.empty:
        st.dataframe(metadata_reference_df, use_container_width=True)
    else:
        st.info("No metadata descriptions available.")
...
with st.sidebar:
    ...
    if len(selected_wafers) == 1:
        _wdesc = wafer_descriptions.get(selected_wafers[0])
        if _wdesc:
            st.caption(f"{selected_wafers[0]}: {_wdesc}")

    selected_patterns = st.multiselect(
        "AJP Patterns", options=all_patterns, default=all_patterns
    )
    if len(selected_patterns) == 1:
        _pdesc = ajp_descriptions.get(selected_patterns[0])
        if _pdesc:
            st.caption(f"{selected_patterns[0]}: {_pdesc}")
with st.expander("Selected code descriptions", expanded=False):
    if selected_wafers:
        st.markdown("**Wafer Types**")
        for wafer in selected_wafers:
            st.caption(
                f"{wafer}: {wafer_descriptions.get(wafer, 'No description available.')}"
            )
    else:
        st.caption("No wafer types selected.")

    if selected_patterns:
        st.markdown("**AJP Patterns**")
        for pattern in selected_patterns:
            st.caption(
                f"{pattern}: {ajp_descriptions.get(pattern, 'No description available.')}"
            )
    else:
        st.caption("No AJP patterns selected.")
    ...
    plot_type = st.selectbox(
        ...
        format_func=lambda x: {
            ...
            "rmsviolin": "RMS channel Violin Subplots",
        }[x],
    )
    ...
    if rms_cols:
        all_rms_vals = pd.concat([df[c] for c in rms_cols]).dropna()
        rms_ch_min = float(all_rms_vals.min()) if not all_rms_vals.empty else 0.0
        rms_ch_max = float(all_rms_vals.max()) if not all_rms_vals.empty else 1000.0
    else:
        rms_ch_min, rms_ch_max = 0.0, 1000.0
rms_channel_range = st.slider(
    "Channel RMS (µV)",
    min_value=rms_ch_min,
    max_value=max(rms_ch_max, rms_ch_min + 1.0),
    value=(rms_ch_min, rms_ch_max),
)
...
if plot_type in metric_map:
    ...
    # ↓ ADD STARTS HERE — still inside the if block, same indentation level
    if st.button("📄 Generate Report"):
        filter_summary = f"""Device Categories: {selected_categories}
Wafer Types: {selected_wafers}
AJP Patterns: {selected_patterns}
Amplifier Boards: {selected_boards}
Yield >= {yield_threshold}%
Device Impedance: {imp_device_range[0]} - {imp_device_range[1]} kOhm
Blind Ch Imp: {round(blind_range[0], 3)} - {round(blind_range[1], 3)}"""

    try:
        # Try PNG export (works locally with Chrome/Kaleido installed)
        try:
            img_bytes = fig.to_image(format="png", width=900, height=500)
            img_b64 = base64.b64encode(img_bytes).decode("utf-8")
            fig_html = f'<img src="data:image/png;base64,{img_b64}" width="100%"/>'
        except Exception:
            # Fallback: embed as interactive Plotly HTML (works everywhere, no Chrome needed)
            fig_html = fig.to_html(full_html=False, include_plotlyjs="cdn")

        # Rest of your HTML report assembly stays the same
        html_report = f"""
        <html><body>
        <h1>TBL Stats Report</h1>
        {fig_html}
        <!-- your tables and model text here -->
        </body></html>
        """
        st.download_button(
            label="⬇️ Download Report",
            data=html_report,
            file_name="stats_report.html",
            mime="text/html",
        )

    except Exception as e:
        st.error(f"Stats report failed:\n\n{e}")
    # ↑ ADD ENDS HERE

elif plot_type == "car_nocar":
    fig = plot_car_nocar_dumbbell(
        temp_df, group_col, f"CAR vs NoCAR by {group_col.replace('_', ' ')}"
    )
    st.plotly_chart(fig, use_container_width=True)
    col_stats = st.columns(2)
    with col_stats[0]:
        st.caption("CAR Summary")
        st.dataframe(
            make_stats_table(temp_df, "CAR_uV_numeric", group_col),
            use_container_width=True,
        )
    with col_stats[1]:
        st.caption("NoCAR Summary")
        st.dataframe(
            make_stats_table(temp_df, "NoCAR_uV_numeric", group_col),
            use_container_width=True,
        )

elif plot_type == "rmsviolin":
    if rms_cols:
        fig = plot_channel_violin_subplots_rms(
            temp_df,
            rms_cols,
            group_col,
            rms_channel_range[0],
            rms_channel_range[1],
            f"Channel RMS Violins by {group_col.replace('_', ' ')}",
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("No RMS channel columns found.")
    ...
    if imp_cols:
        ...
        if st.button("📄 Generate Report", key="report_box"):
            filter_summary = f"""Device Categories: {selected_categories}
Wafer Types: {selected_wafers}
AJP Patterns: {selected_patterns}
Amplifier Boards: {selected_boards}
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
    try:
        # Try PNG export (works locally with Chrome/Kaleido installed)
        try:
            img_bytes = fig.to_image(format="png", width=900, height=500)
    ...
    if imp_cols:
        ...
        if st.button("📄 Generate Report", key="report_kde"):
            filter_summary = f"""Device Categories: {selected_categories}
Wafer Types: {selected_wafers}
AJP Patterns: {selected_patterns}
Amplifier Boards: {selected_boards}
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
    ...
    if imp_cols:
        ...
        st.plotly_chart(fig, use_container_width=True)
        if st.button("📄 Generate Report", key="report_violin"):
            filter_summary = f"""Device Categories: {selected_categories}
Wafer Types: {selected_wafers}
AJP Patterns: {selected_patterns}
Amplifier Boards: {selected_boards}
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
    ...
    if imp_cols:
        ...
        if st.button("📄 Generate Report", key="report_low"):
            filter_summary = f"""Device Categories: {selected_categories}
Wafer Types: {selected_wafers}
Wafer Prefix: {selected_prefix}
Amplifier Boards: {selected_boards}"""
    try:
        # Try PNG export (works locally with Chrome/Kaleido installed)
        try:
    ...
    if len(temp_df) == 0:
        ...
        try:
            if test_type == "t-test (two groups)":
                ...
                st.session_state["stats_fig"] = fig_reg
                st.session_state["stats_model_text"] = result["model_text"]
                st.text(result["model_text"])
                ...
                st.session_state["stats_fig"] = fig_glm
                st.session_state["stats_model_text"] = result["model_text"]
                st.text(result["model_text"])
            ...
            st.exception(e)
if st.button("📄 Generate Stats Report", key="report_stats"):
    filter_summary = f"""Test Type: {test_type}
Response Variable: {response_col}
Grouping Column: {stat_group_col}
Rows used: {len(temp_df)}"""

    try:
        fig_section = ""
        model_section = ""

        if "stats_fig" in st.session_state:
            try:
                img_bytes = st.session_state["stats_fig"].to_image(
                    format="png", width=900, height=500
                )
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                fig_section = f'<h2>Plot</h2><img src="data:image/png;base64,{img_b64}" width="100%"/>'
            except Exception:
                fig_section = "<h2>Plot</h2>" + st.session_state["stats_fig"].to_html(
                    full_html=False, include_plotlyjs="cdn"
                )

        if "stats_model_text" in st.session_state:
            model_section = f"<h2>Model Summary</h2><pre>{st.session_state['stats_model_text']}</pre>"
        ...
        <h1>TBL Statistical Analysis Report</h1>
        <p><b>Generated:</b> {pd.Timestamp.now().strftime("%Y-%m-%d %H:%M")}</p>
        ...
        {temp_df.describe().to_html(border=1)}
        </body></html>
        """
        st.download_button(
            label="⬇️ Download Stats Report as HTML",
            data=stats_html.encode("utf-8"),
            file_name="TBL_stats_report.html",
            mime="text/html",
            key="dl_stats",
        )
    except Exception as e:
        st.error(f"Stats report failed: {e}")
