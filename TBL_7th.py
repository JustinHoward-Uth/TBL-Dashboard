...
Blind Ch Imp: {round(blind_range[0], 3)} - {round(blind_range[1], 3)}"""
        ...
        try:
            ...
            stats_df = make_stats_table(temp_df, metric_col, group_col)
            report_html = generate_report_html(
                filter_summary, stats_df, img_b64, metric_label
            )
            st.download_button(
                label="⬇️ Download Report as HTML",
                data=report_html.encode("utf-8"),
                file_name="TBL_report.html",
                mime="text/html",
            )
        except Exception as e:
            st.error(f"Report generation failed: {e}. Make sure kaleido is installed.")
    # ↑ ADD ENDS HERE
...
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
            try:
                img_bytes = fig.to_image(format="png", width=900, height=500)
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                stats_df = make_stats_table(imp_long, "Imp_Ohm", group_col)
                report_html = generate_report_html(
                    filter_summary, stats_df, img_b64, "Impedance Box + Scatter"
                )
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
                    file_name="TBL_report_box.html",
                    mime="text/html",
                    key="dl_box",
                )
            except Exception as e:
...
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
            try:
                ...
                stats_df = make_stats_table(imp_long, "Imp_Ohm", group_col)
                report_html = generate_report_html(
                    filter_summary, stats_df, img_b64, "KDE Violin"
                )
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
                    file_name="TBL_report_kde.html",
                    mime="text/html",
                    key="dl_kde",
                )
            except Exception as e:
...
Channel Impedance: {imp_channel_range[0]} - {imp_channel_range[1]} Ohm"""
            try:
                ...
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                report_html = generate_report_html(
                    filter_summary, pd.DataFrame(), img_b64, "Channel Violin Subplots"
                )
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
                    file_name="TBL_report_violin.html",
                    mime="text/html",
                    key="dl_violin",
                )
            except Exception as e:
...
Amplifier Boards: {selected_boards}"""
            try:
                ...
                img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                report_html = generate_report_html(
                    filter_summary, low_table, img_b64, "Devices with Any Channel < 5kΩ"
                )
                st.download_button(
                    label="⬇️ Download Report as HTML",
                    data=report_html.encode("utf-8"),
                    file_name="TBL_report_low_imp.html",
                    mime="text/html",
                    key="dl_low",
                )
            except Exception as e:
...
Rows used: {len(temp_df)}"""
    ...
    try:
        ...
        # Include plot if one was generated (Linear Regression or GLM)
