...
if hint:
    with st.expander("⚡ Power Analysis planner", expanded=False):
        ...
        with mid2:
            sided = st.radio("Test type", ["Two-sided", "One-sided"], index=0, horizontal=True)
            ratio = st.slider(
        ...
        if not np.isnan(eff_d) and eff_d != 0:
            ...
            eff_grid = [max(0.1, round(eff_d * f, 3)) for f in [0.5, 0.75, 1.0, 1.25, 1.5]]
            rows = []
            for d_eff in eff_grid:
                z_alpha_g = z_alpha
                z_beta_g = z_beta
                n1_g = math.ceil(((z_alpha_g + z_beta_g) ** 2) * (1 + 1 / ratio) / (d_eff ** 2))
                n2_g = math.ceil(n1_g * ratio)
                rows.append({"d": d_eff, "N_baseline": n1_g, "N_candidate": n2_g})
            sens_df = pd.DataFrame(rows)
            st.dataframe(sens_df, use_container_width=True, hide_index=True)
        else:
            st.warning("Cannot estimate sample size — effect size is zero or unavailable. Try entering a non-zero d.")
